<?php
   ...
   $consulta = $pdo->prepare("SELECT * FROM carrinho_temporario WHERE temporario_ptoduto = 
   ...
   if($linhas >= 1):
      ...
      $altera -> execute();
      if($altera):
         echo '<script>alert("Foi adicionada mais uma unidade a este produto")</script>';
         echo '<script>window.location="indexcarrinho.php"</script>';
      else:
         echo '<script>alert("Este produto não pode ser adicionado ao carrinho")</script>';
         echo '<script>window.location="indexcarrinho.php"</script>';
      endif;

   else:
      $inserir = $pdo->prepare("INSERT INTO carrinho_temporario (temporario_produto, temporario_qtde, temporario_preco, temporario_data, temporario_sessao") 
      VALUES (:produto, :qtde, :preco, :data, :sessao)");
